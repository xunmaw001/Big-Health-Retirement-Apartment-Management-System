import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import news from '@/views/modules/news/list'
    import tuizhuxinxi from '@/views/modules/tuizhuxinxi/list'
    import tixingxinxi from '@/views/modules/tixingxinxi/list'
    import fangjianxinxi from '@/views/modules/fangjianxinxi/list'
    import fangjianleixing from '@/views/modules/fangjianleixing/list'
    import ruzhuxinxi from '@/views/modules/ruzhuxinxi/list'
    import jiedaixinxi from '@/views/modules/jiedaixinxi/list'
    import hulixinxi from '@/views/modules/hulixinxi/list'
    import yonghu from '@/views/modules/yonghu/list'
    import qingjiaxinxi from '@/views/modules/qingjiaxinxi/list'
    import hugong from '@/views/modules/hugong/list'
    import shoufeibiaozhun from '@/views/modules/shoufeibiaozhun/list'
    import xuzhuxinxi from '@/views/modules/xuzhuxinxi/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '系统首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '系统首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/news',
        name: '公告信息',
        component: news
      }
      ,{
	path: '/tuizhuxinxi',
        name: '退住信息',
        component: tuizhuxinxi
      }
      ,{
	path: '/tixingxinxi',
        name: '提醒信息',
        component: tixingxinxi
      }
      ,{
	path: '/fangjianxinxi',
        name: '房间信息',
        component: fangjianxinxi
      }
      ,{
	path: '/fangjianleixing',
        name: '房间类型',
        component: fangjianleixing
      }
      ,{
	path: '/ruzhuxinxi',
        name: '入住信息',
        component: ruzhuxinxi
      }
      ,{
	path: '/jiedaixinxi',
        name: '接待信息',
        component: jiedaixinxi
      }
      ,{
	path: '/hulixinxi',
        name: '护理信息',
        component: hulixinxi
      }
      ,{
	path: '/yonghu',
        name: '用户',
        component: yonghu
      }
      ,{
	path: '/qingjiaxinxi',
        name: '请假信息',
        component: qingjiaxinxi
      }
      ,{
	path: '/hugong',
        name: '护工',
        component: hugong
      }
      ,{
	path: '/shoufeibiaozhun',
        name: '收费标准',
        component: shoufeibiaozhun
      }
      ,{
	path: '/xuzhuxinxi',
        name: '续住信息',
        component: xuzhuxinxi
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '系统首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})
const originalPush = VueRouter.prototype.push
//修改原型对象中的push方法
VueRouter.prototype.push = function push(location) {
   return originalPush.call(this, location).catch(err => err)
}
export default router;
