<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'" label="接待编号" prop="jiedaibianhao">
					<el-input v-model="ruleForm.jiedaibianhao" placeholder="接待编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.jiedaibianhao" label="接待编号" prop="jiedaibianhao">
					<el-input v-model="ruleForm.jiedaibianhao" placeholder="接待编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="接待时间" prop="jiedaishijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.jiedaishijian" label="接待时间" prop="jiedaishijian">
					<el-input v-model="ruleForm.jiedaishijian" placeholder="接待时间" readonly></el-input>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.jiedaishijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='jiedaibianhao'){
							this.ruleForm.jiedaibianhao = obj[o];
							this.ro.jiedaibianhao = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					var json = data.data;
					if(json.hugongzhanghao!=''&&json.hugongzhanghao && this.$storage.get("role")!="管理员"){
						this.ruleForm.hugongzhanghao = json.hugongzhanghao
						this.ro.hugongzhanghao = true;
					}
					if(json.hugongxingming!=''&&json.hugongxingming && this.$storage.get("role")!="管理员"){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../springbootto14d/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiedaixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiedaixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
